...
## ---------
## The following code generates a plot of Global Active Power from meters on an hourly basis 
...
## Generates histogram of Global Active Power
hist_global_active_power <- function(dtMeter) {
  hist(dtMeter$Global_active_power,
...
plot1 <- function(toPNG = TRUE){
  ...
  if(toPNG) { png(file = "plot1.png", bg="transparent", width = 480, height = 480, units ="px")}
  
  ## Generate the Plot
  hist_global_active_power(dtMeter)
